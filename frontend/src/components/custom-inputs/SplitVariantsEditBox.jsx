...
import './SplitVariantsEditBox.css';

function SplitVariantsEditBox({
  value,
  parts,
  onChange,
  onSave,
  ...
  useEffect(() => {
    if (value !== null && value !== undefined) {
      ...
      // When entering edit mode, initialize with the previous value (if editing existing) or empty object (if new)
      const initialValue = {};
      if (previousValueRef.current) {
        parts.forEach(part => {
          initialValue[part] = previousValueRef.current[part] || '';
        });
      } else {
        parts.forEach(part => {
          initialValue[part] = '';
  ...
  }, [isEditing, value, parts]);

  const handleSave = () => {
    // Validate all parts are filled
    if (editingValue && parts.every(part => editingValue[part]?.trim())) {
      onSave?.(editingValue);
    }
  };

  const handleKeyPress = (e, partIndex) => {
    if (e.key === 'Enter' && partIndex === parts.length - 1) {
      // If Enter is pressed on the last part, save
      handleSave();
    }
  };
  ...
  if (isEditing) {
    return (
      <div className="split-variant-edit-box editing">
        <div className="split-variant-inputs">
          {parts.map((part, index) => (
            <div key={part} className="split-part-input">
              <label className="split-part-label">
                <strong>{part}:</strong>
              </label>
              <input
                type="text"
                value={editingValue[part] || ''}
        ...
        <IconGreyButton
          ...
          onClick={handleSave}
          ariaLabel="Save"
          disabled={!editingValue || !parts.every(part => editingValue[part]?.trim()) || disabled}
        />
      </div>
    );
  }
  ...
  return (
    <div className="split-variant-edit-box filled">
      <div className="variant-content">
        <div className="split-variant-parts">
          {parts.map(part => (
            <div key={part} className="split-part-display">
              <strong>{part}:</strong> {value[part] || ''}
            </div>
          ))}
        </div>
        <IconGreyButton
...
export default SplitVariantsEditBox;
